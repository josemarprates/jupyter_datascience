...
def obter_nota_de_risco(cpf):
    ...
    if cliente.empty:
        ...
        # Critérios de feedback ajustados
        if nota < 30:
            faixa_risco = "Baixo Risco"
            descricao = "Cliente com alta chance de pagamento e baixo risco de inadimplência."
        elif 30 <= nota <= 70:
            faixa_risco = "Risco Moderado"
            descricao = "Cliente com risco intermediário de inadimplência."
        else:
            faixa_risco = "Alto Risco"
            descricao = "Cliente com alto risco de inadimplência."

        # Histórico de compras e renegociação
